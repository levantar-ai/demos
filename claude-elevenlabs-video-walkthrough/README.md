# Claude + Playwright + ElevenLabs → narrated MP4 walkthrough

A sibling of `claude-polly-video-walkthrough/`. The flow being captured is
identical — the same Acme contact form, the same seven steps — the only
difference is the voice: **ElevenLabs** synthesizes the narration instead of
AWS Polly.

> **Per-step CSS spotlights highlight whichever element the narrator is
> talking about. Claude+Playwright captures the frames. ElevenLabs (George, a
> warm British storyteller voice) narrates. ffmpeg stitches the result into
> one video.**
...
walkthrough/
  ...
  narrate.mjs         Reads the manifest, calls the ElevenLabs text-to-speech
                      API per step (voice George, model eleven_multilingual_v2),
                      writes presentation/audio/NN-name.mp3.
...
declares the caption, the narration text ElevenLabs reads, *and* the CSS
...
# 3. Narrate with ElevenLabs. Needs an API key:
...
There's also an `npm run video` convenience that chains capture+narrate+overlays+render
(it reads `ELEVENLABS_API_KEY` from the environment for the narrate stage).

## Requirements

- `ffmpeg` and `ffprobe` on $PATH (used by `render-video.mjs`).
- An ElevenLabs API key in `ELEVENLABS_API_KEY` with text-to-speech access.
- Voice is George (`JBFqnCBsd6RMkjVDRZzb`), a British storyteller voice, with
  the `eleven_multilingual_v2` model — already wired in `steps.json` and read
  by `narrate.mjs`. To swap voices, edit `voice` at the top of the manifest
...
```js
{
  ...
  narration: 'What ElevenLabs reads aloud, in plain prose.',
