...
const resolve = (v, fallback) => (v ? (v.startsWith('/') ? v : join(ROOT, v)) : fallback);
// MANIFEST / CLIPS_DIR / OUTPUT may be overridden to render a localized variant
// into its own clips dir and final file (e.g. walkthrough-fr.mp4).
const MANIFEST = resolve(process.env.MANIFEST, join(ROOT, 'steps.json'));
const CLIPS = resolve(process.env.CLIPS_DIR, join(ROOT, 'clips'));
const OUTPUT = resolve(process.env.OUTPUT, join(ROOT, 'walkthrough.mp4'));
