"""Brightwell's order agent, post 04: running code in a sandbox.

Runtime HTTP contract as post 01 (POST /invocations, GET /ping). Post a CSV
and the agent writes it into a Code Interpreter session and runs pandas
over it there, so the file is parsed away from the agent's own microVM and
its credentials. The code is fixed and written here. There is no model in
this agent; the routing below is code, and the sandbox is called because
the handler calls it. Post 06 is where a model is handed these tools.
"""
...
# Runs inside the sandbox. Deliberately plain, the post is about where this
...
def _consume(response):
    ...
    The whole stream is read before raising, so a failure never leaves
    events unconsumed on the connection, and the error text is whatever the
    tool put in the erroring event.
    """
    chunks, error = [], None
