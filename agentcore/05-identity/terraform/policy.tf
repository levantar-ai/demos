...
# The customer is whoever the presented token's customer_id claim names, so
# a call whose customer_id argument differs from that claim is denied here
# regardless of what the agent or a model decided to ask for. The engine
# cannot tell which invocation supplied the token; that is the post's stated
# limit.
...
# Cedar. The principal is the token's sub, the exchange pool's service user,
# and every claim is a principal tag, including customer_id, which the
# exchange pool's pre-token trigger set from the customer's verified token.
# The tool's customer_id argument is context.input.customer_id. This permits
# list_orders only when the argument matches the customer_id in the presented
# token, and Cedar is default-deny, so a mismatched customer_id has no permit
# and is refused. hasTag first: a token without the claim must not error into
# a decision.
...
# argument is not the customer_id in the presented token.
